...
void main() {
  ...
  group('QuizService', () {
    ...
    test('getPercentageScore calculates correct percentage', () {
      ...
      final percentage = quizService.getPercentageScore(
        score,
        totalQuestions,
    ...
    test('calculateScoreWithMethod straight scoring', () {
      ...
      final answers = {
        ...
        1: [
          ...
          false,
          false,
        ], // Only answered 1 branch, 4 correct, 1 wrong = 4 points
      };

      final score = quizService.calculateScoreWithMethod(
        quiz,
        answers,
        ScoringMethod.straight,
      );
      expect(
        score,
        8,
      ); // 5 + 3 = 8 (answered branch C wrong, but didn't answer A,B,D,E so they don't count)
    });
    ...
    test('calculateScoreWithMethod minus not carried over scoring', () {
      ...
      final answers = {
        ...
        1: [
          ...
          false,
          false,
        ], // 0 correct, 1 wrong: 0-1=-1, clamped to 0, plus 4 unanswered (don't count) = 1 point total
      };

      final score = quizService.calculateScoreWithMethod(
        quiz,
        answers,
        ScoringMethod.minusNotCarriedOver,
      );
      expect(
        score,
        6,
      ); // 5 + 1 = 6 (Q2: answered C wrong but min is 0 for the stem, actually it counts answered branches)
    });
    ...
    test('calculateScoreWithMethod minus carried over scoring', () {
      ...
      final answers = {
        0: [
          ...
          true,
          true,
        ], // 1 correct (A), 4 wrong (B,C,D,E): 1-4=-3
      ...
      final score = quizService.calculateScoreWithMethod(
        quiz,
        answers,
        ScoringMethod.minusCarriedOver,
      );
